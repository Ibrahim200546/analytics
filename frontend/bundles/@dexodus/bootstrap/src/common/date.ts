...
import {arrayChunks} from "./array";
import {DateInterval, DateValue} from "../helpers/DateIntervalHelper/DateIntervalHelper";
...
export const countDaysInDateInterval = (interval: DateInterval): number => {
    const { start, end } = interval;

    // Преобразуем значения start и end в Date, если возможно
    const startDate = mapDateValueToDate(start);
    const endDate = mapDateValueToDate(end);

    // Если одно из значений null или некорректно, возвращаем 0
    ...
    // Преобразуем миллисекунды в дни (1 день = 86 400 000 миллисекунд)
    const diffInDays = diffInMs / (1000 * 60 * 60 * 24);

    // Возвращаем абсолютное значение количества дней и округляем
    return Math.max(Math.floor(diffInDays) + 1, 0); // +1 учитывает оба дня как полный интервал
};
